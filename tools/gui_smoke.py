...
def smoke() -> list[str]:
    ...
    try:
        ...
        app.main_notebook.select(app.inputs_tab)
        app.update()
        ...
        require(len(app.main_notebook.tabs()) == 4, "Las pestanas Report y Help deben iniciar ocultas")
        require(str(app.report_tab) not in app.main_notebook.tabs(), "Report debe iniciar oculta")
        require(str(app.help_tab) not in app.main_notebook.tabs(), "Help debe iniciar oculta")
        app.report_tab_visible_var.set(True)
        app.toggle_report_tab()
        require(str(app.report_tab) in app.main_notebook.tabs(), "No se pudo mostrar Report")
        app.help_tab_visible_var.set(True)
        app.toggle_help_tab()
        require(str(app.help_tab) in app.main_notebook.tabs(), "No se pudo mostrar Help")
        require("w <word> <hex value>" in app.help_text.get("1.0", "end"), "Help no contiene referencia de comandos")
        app.report_tab_visible_var.set(False)
        app.toggle_report_tab()
        app.help_tab_visible_var.set(False)
        app.toggle_help_tab()
        require(len(app.main_notebook.tabs()) == 4, "No se pudieron ocultar las pestanas opcionales")
